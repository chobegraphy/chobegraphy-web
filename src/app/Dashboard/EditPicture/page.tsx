...
import Banner from "@/components/PageWise/EditPicture/Banner";
import CategorySelector from "@/components/PageWise/EditPicture/CategorySelector";
import CopyRightType from "@/components/PageWise/EditPicture/CopyRightType";
import PhotoMetaData from "@/components/PageWise/EditPicture/PhotoMetaData";
import UploadInfo from "@/components/PageWise/EditPicture/UploadInfo";
...
const EditPhotoPage = () => {
    ...
    const [exifData, setExifData] = useState(data?.exifData);
    ...
    return (
        <div className="grid grid-cols-10 max-lg:grid-cols-1 sm:px-10 px-5 py-5  dark:bg-gradient-to-br from-black to-light-primary-color gap-5 h-full">
            <Banner imgData={data} SelectedCopyrightType={SelectedCopyrightType} setSelectedCopyrightType={setSelectedCopyrightType} colors={colors} setColors={setColors} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} exifData={exifData} setExifData={setExifData} />
                ...
                ><svg className="w-14  h-14 max-md:w-10 max-md:h-10" viewBox="0 0 73 60" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span className="">
                        <p className={`${Language === "BN" && "font-BanglaHeading"} text-lg`}>
                            {Language === "BN" && "আবার ছবি আপলোড করুন"}
                        </p>
                        {Language === "EN" && <span className="font-Righteous text-xl">Upload Picture Again</span>}
                    </span>
