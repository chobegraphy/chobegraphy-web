...
const PhotoDetails = ({ description, setDistrict1, district, setDistrict, download, view, react, register, DetailsData, setDescription, uploadedTime, colors, dimensions, fileSize, onSubmit, }: any) => {
  // auth data
      ...
      <section className="  relative text-light-primary-color dark:text-dark-primary-color mt-2">
        ...
        <div className="flex flex-col gap-y-1">
          ...
          <h1
            ...
            {colors !== undefined && <ul style={{ borderTop: "none", border: `2px solid ${showDropdown && colors !== undefined && filteredZilla?.length > 0 ? colors[1]?.hex : colors[1]?.hex}` }} className={`${showDropdown && filteredZilla.length > 0 ? "border  rounded-xl  rounded-t-none border-t-0 h-full opacity-100 z-10" : " rounded-xl h-0 opacity-0 -z-20"}  border   rounded-xl transform duration-300 max-h-52 w-full border-light-secondary-color  overflow-y-scroll example`}>
