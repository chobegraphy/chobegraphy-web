    ...
    <>
      ...
      <div className="py-10 justify-center flex gap-x-1.5 items-center">
        ...
        <button onClick={() => {
          ...
          router.push(`/ViewImgProfile?AuthorMail=${authorEmail}&status=${status}&CurrentPage=${page - 1}`);
        ...
        </button>
        {
          [...Array(totalPages)]?.map((page, i) => (
            <button key={i + 1} onClick={() => {
              window.scrollTo({ top: 0, behavior: 'smooth' });
              router.push(`/ViewImgProfile?AuthorMail=${authorEmail}&status=${status}&CurrentPage=${i + 1}`);
              setCurrentPage(i)
